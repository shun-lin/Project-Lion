...
class AttentionQKV(Model):
    ...
    def call(self, queries, keys, values, mask=None):
        ...
        similarity = tf.linalg.matmul(queries, keys, transpose_b=True) # Compute the similarity according to the QKV formula
        ...
        output = tf.linalg.matmul(weights, values) # Obtain the output
        ####################################  END OF YOUR CODE  ##################################
...
class MultiHeadProjection(Model):
    ...
    def _split_heads(self, tensor):
        ...
        tensor = tf.reshape(tensor, shape=(batch_size, tensorlen, self.n_heads, new_feature_size))
    ...
    def _combine_heads(self, tensor):
        ...
        tensor = tf.reshape(tensor, shape=(batch_size, tensorlen, new_feature_size)) # Reshape the Tensor to remove the heads dimension and come back to a Rank-3 tensor
        return tensor
        ##########################################################################################
